...
void init(int argc, char *argv[]){
    if(argc < 6){
        printf("usage: mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4 ./mandelbrot_ompi_pth c_x_min c_x_max c_y_min c_y_max image_size nthreads\n");
        printf("examples with image_size = 11500:\n");
        printf("    Full Picture:         mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4 ./mandelbrot_ompi_pth -2.5 1.5 -2.0 2.0 11500 2\n");
        printf("    Seahorse Valley:      mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4 ./mandelbrot_ompi_pth -0.8 -0.7 0.05 0.15 11500 2\n");
        printf("    Elephant Valley:      mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4 ./mandelbrot_ompi_pth 0.175 0.375 -0.1 0.1 11500 2\n");
        printf("    Triple Spiral Valley: mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4 ./mandelbrot_ompi_pth -0.188 -0.012 0.554 0.754 11500 2\n");
        exit(0);
    }
    else{
...
int main(int argc, char *argv[]){
    ...
    // Nota: compilar com --use-hwthread-cpus. E.g.:
    // mpirun --use-hwthread-cpus -np 4  --bind-to none ./mandelbrot_ompi_omp -2.5 1.5 -2.0 2.0 8192 2
    ...
    if (taskid == MASTER) {
        ...
        printf("Opth %d %d %.3lf ", numtasks, nthreads, elapsedTime(ts, tf));
        write_to_file();
    }
