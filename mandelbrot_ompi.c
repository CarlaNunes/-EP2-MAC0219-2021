...
void init(int argc, char *argv[]){
    if(argc < 6){
        printf("usage: mpirun mpirun --oversubscribe --use-hwthread-cpus --bind-to none -np 4  ./mandelbrot_ompi c_x_min c_x_max c_y_min c_y_max image_size\n");
        printf("examples with image_size = 11500:\n");
        printf("    Full Picture:         mpirun -np 4 ./mandelbrot_ompi -2.5 1.5 -2.0 2.0 11500\n");
        printf("    Seahorse Valley:      mpirun -np 4 ./mandelbrot_ompi -0.8 -0.7 0.05 0.15 11500\n");
        printf("    Elephant Valley:      mpirun -np 4 ./mandelbrot_ompi 0.175 0.375 -0.1 0.1 11500\n");
        printf("    Triple Spiral Valley: mpirun -np 4 ./mandelbrot_ompi -0.188 -0.012 0.554 0.754 11500\n");
        exit(0);
    }
    else{
...
void update_rgb_buffer() {
    ...
    for (int i = 1; i < numtasks; i++) {
        ...
        //printf("MASTER: Sending  permission to task %d.\n",i);
        ...
        //printf("MASTER: Array received from task %d.\n",i);

        for (int x = i; x < i_x_max; x += numtasks) {
            for (int y = 0; y < i_y_max; y++) {
                iteration = a_iteration[(i_x_max *(x/numtasks)) + y];
                //printf("iteration(%d,%d) = %d at %d\n",x,y, iteration, (i_x_max *(x/numtasks)) + y);
                int color;
...
void compute_mandelbrot(){
    ...
    for(i_x = taskid; i_x < i_x_max; i_x += numtasks) {
...
int main(int argc, char *argv[]){
    ...
    // mpirun --use-hwthread-cpus -np 4 ./mandelbrot_ompi_omp -2.5 1.5 -2.0 2.0 8192
    ...
    if (taskid == MASTER) {
        ...
        printf("Ompi %d 1 %.3lf ", numtasks, elapsedTime(ts, tf));
        write_to_file();
    }
